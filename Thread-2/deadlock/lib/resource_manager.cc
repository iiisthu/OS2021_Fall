...
bool ResourceManager::safe(int GPU_amount, int MEMORY_amount, int DISK_amount, int NETWORK_amount, std::map<std::thread::id, int> GPU_request, std::map<std::thread::id, int> MEMORY_request, std::map<std::thread::id, int> DISK_request, std::map<std::thread::id, int> NETWORK_request, std::map<std::thread::id, int> GPU_alloc, std::map<std::thread::id, int> MEMORY_alloc, std::map<std::thread::id, int> DISK_alloc, std::map<std::thread::id, int> NETWORK_alloc) {
    std::map<std::thread::id, int> GPU_request_c = GPU_request;
    std::map<std::thread::id, int> MEMORY_request_c = MEMORY_request;
    std::map<std::thread::id, int> DISK_request_c = DISK_request;
    std::map<std::thread::id, int> NETWORK_request_c = NETWORK_request;
    ...
    while (possible) {
        ...
        for (iter = GPU_request_c.begin(); iter != GPU_request_c.end(); iter++) {
            std::thread::id this_id = iter->first;
            if (is_suspend[this_id]==true) {
                flag = true;
                std::cout<<"%%"<<std::this_thread::get_id()<<is_suspend[std::this_thread::get_id()]<<std::endl;
                GPU_request_c.erase(iter);
                break;
            }
            if (GPU_request_c[this_id] <= GPU_amount && MEMORY_request_c[this_id] <= MEMORY_amount && DISK_request_c[this_id] <= DISK_amount && NETWORK_request_c[this_id] <= NETWORK_amount) {
                flag = true;
    ...
    if (GPU_request_c.size() == 0) return true;
    return false;
}
